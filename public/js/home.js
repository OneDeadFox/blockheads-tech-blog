...
bodyEvents.addEventListener(`click`, async (e) => {
    ...
    blogBlock = e.target.closest(`.block-party`);
    newBitDisp = e.target.closest(`.add-btn`);
    oldBitsDisp = e.target.closest(`.old-btn`);
    bitSubmitBtn = e.target.closest(`.bit-sub-btn`);
    ...
    const submitBtnEl = document.querySelector(`#bit-submit-btn`);

    if(blogBlock){
        //get current block infor from click event
        const blockTitle = blogBlock.children[0].children[0].innerText;
        const blockContent = blogBlock.children[0].children[1].innerText;
        const blockDate = blogBlock.children[0].children[2].innerText;
        let blockUserRaw = blogBlock.children[0].children[3].innerText;
        blockUserRaw = blockUserRaw.match(/(?<=-)\w+\b/);
        const blockUser = blockUserRaw[0];
        console.log(blogBlock);
        blockId = blogBlock.children[0].id; 
